...
const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/nvustatus');
  ...
  const handleAddModalOk = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/nvustatus', newItem);
      // Kiểm tra phản hồi
      console.log(response.data); // Xem phản hồi từ API
      console.log(dataSource);
      // Giả sử response.data.data chứa bản ghi mới đã được tạo
      if (!response.data.error) {
        setDataSource(response.data.data); // Cập nhật dataSource với item mới
  ...
  const handleModalOk = async () => {
    try {
      ...
      await axios.put(`http://localhost:8000/api/nvustatus/${currentItem.status_id}`, updatedItem);
      message.success('Updated successfully');
      setDataSource((prevData) =>
        prevData.map((item) => (item.status_id === currentItem.status_id ? { ...item, ...updatedItem } : item)),
      );
  ...
  //   try {
  //     await axios.delete(`http://localhost:8000/api/nvustatus/${statusId}`);
  //     message.success('Deleted successfully');
  //     setDataSource((prevData) => prevData.filter((item) => item.status_id !== statusId));
  //   } catch (error) {
  ...
  // Cột cho bảng
  const columns = [
    { title: 'ID', dataIndex: 'status_id', key: 'status_id' },
    { title: 'Tên trạng thái', dataIndex: 'status_name', key: 'status_name' },
    {
      title: 'Màu ',
      ...
      <Main>
        ...
        >
          <Form form={form} layout="vertical">
            ...
            <Form.Item name="status" initialValue={1} hidden>
              <Input type="hidden" />
            </Form.Item>
          </Form>
        </Modal>
