...
function ProjectLists({ listProject, listUser = [] }) {
  ...
  if (projects?.length)
    projects.map((value, index) => {
      ...
      return dataSource.push({
        ...
        // eslint-disable-next-line camelcase
        project_start_date: <span className="date-started">{moment(project_start_date).format('DD-MM-YYYY')}</span>,
        // eslint-disable-next-line camelcase
        project_end_date: <span className="date-finished">{moment(project_end_date).format('DD-MM-YYYY')}</span>,
        project_members: (
          <ProjectListAssignees>
            <ul>
              ...
              {project_members.length > 5 && (
                ...
                >
                  <li>
                    <Avatar width={30} height={30} name={`+ ${project_members.length - 5}`} />
                  </li>
                </div>
              )}
            </ul>
        ...
        project_status: (
          ...
          >
            {checkStatus(project_status)?.status}
          </Tag>
        ),
        success: (
          <div className="project-list-progress">
            ...
            <p>
              {' '}
              {completed_tasks}/{total_tasks} Hoàn thành
            </p>
          </div>
        ),
        action: (
          <Dropdown
            ...
            content={
              <div className="popover-content">
                ...
                <div className="action-item" onClick={() => handleEditClick('status', value)}>
                ...
                <div className="action-item" onClick={() => handleEditClick('start_date', value)}>
                  <MdOutlineDateRange size={30} className="d-block ms-1 fs-4 text-secondary" />
                  <span>Sửa ngày bắt đầu</span>
                </div>
                <div className="action-item" onClick={() => handleEditClick('end_date', value)}>
                  <MdOutlineDateRange size={30} className="d-block ms-1 fs-4 text-secondary" />
  ...
  return (
    <Row gutter={25}>
      ...
      <Modal visible={showModalUpdateMembers} onCancel={handleCancel} centered title="Thành viên" footer={null}>
          ...
          <>
            <div style={{ marginBottom: '16px' }}>
              {selectedMembers.length > 0 && (
                <>
                  <h6 style={{ marginBottom: '8px', fontWeight: 'bold', fontSize: '1.1rem' }}>Thành viên đã chọn:</h6>
                  <div
                    style={{
                      display: 'flex',
                      flexWrap: 'wrap',
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: '16px' }}>
          {showMemberExit ? (
            <Button type="primary" style={{ minWidth: '300px' }} onClick={handleUpdateMembers}>
              {loadingUpdate ? <Spin /> : 'Cập nhật'}
            </Button>
          ) : (
            <Button type="primary" style={{ minWidth: '300px' }} onClick={handleJoinProject}>
